      ...
      <tr key={user._id}>
        ...
        <td>
          {user.qualities.map((qualiti) => (
            <span class={`badge m-1 rounded-pill bg-${qualiti.color}`}>
              {qualiti.name}
            </span>
          ))}
        </td>
        <td>
          ...
          {renderPhrase(calculateAge(user.birthday), "год", "года", "лет")})
        </td>
        <td>{user.profession.name}</td>
        <td>
          <a className="link-primary" href={`tel:${user.phone}`}>{user.phone}</a>
        </td>
        <td><a className="link-primary" href={`mailto::${user.mail}`}>{user.mail}</a></td>
        <td>
          <button
  ...
  return (
    <div className="m-5">
      <p className="mb-4 fs-2">
        {users.length !== 0
          ? renderPhrase(users.length, "юзер", "юзера", "юзеров", "в базе")
